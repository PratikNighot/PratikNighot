## 👋 Hi, I'm Pratik Nighot!

> 💡 "Building scalable, intelligent systems that think, act, and evolve."

**Senior Backend Software Engineer** | **Java • Spring Boot • Microservices • Kafka • AWS**  
**Passionate about AI, LLMs & Agentic Systems**

---

## 🚀 About Me

~5 years of experience architecting **enterprise-grade, cloud-native distributed systems**.  
Specialized in **event-driven microservices**, real-time integrations, and high-availability platforms.  
Currently obsessed with **LLMs, Agentic AI, autonomous agents, and intelligent workflows**.

- 🔭 Senior Backend Engineer @ **24/7 Software** (Crest Venue & Entertainment Analytics Platform)
- 🌱 Deeply exploring **LangChain, CrewAI, multi-agent systems, RAG & LLM orchestration**
- 💬 Ask me about **System Design, Event Streaming, Kafka Architecture, or AI Agents**

---

## 🛠️ Tech Stack

**Languages**  
Java 21 (Expert) • Python • SQL • Shell Script

**Backend & Frameworks**  
Spring Boot • Microservices • JPA/Hibernate • REST • SOAP • gRPC • Kafka (Producers, Consumers, DLQ, Retry)

**AI / LLM Stack**  
Anthropic Claude • LangChain • CrewAI • Agentic AI Patterns

**Databases**  
MySQL • PostgreSQL • MongoDB • Redis

**Cloud & DevOps**  
AWS (EC2, S3, Lambda) • Docker • Kubernetes • Jenkins • GitHub Actions • CI/CD

**Monitoring & Architecture**  
Datadog • Prometheus • ELK • System Design (HLD/LLD) • Event-Driven Architecture • Netflix Conductor

---

## 📊 GitHub Stats

![Pratik Nighot's GitHub Stats](https://github-readme-stats.vercel.app/api?username=PratikNighot&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=PratikNighot&layout=compact&theme=tokyonight)

![Contribution Streak](https://github-readme-streak-stats.herokuapp.com/?user=PratikNighot&theme=tokyonight)

---

## 🐍 Contribution Snake

![Contribution Snake](https://github.com/PratikNighot/PratikNighot/blob/output/github-contribution-grid-snake.svg)

---

## 🔥 Featured Projects

**War Room — AI Interview Prep Dashboard**  
Full-stack AI-powered interview preparation platform using **Anthropic Claude API**  
→ DSA tracking, System Design notes & Behavioural prep  
[Live Demo](https://war-room-virid.vercel.app)

**Algorithmic Trading System** (Personal)  
Real-time market data ingestion, ML ensemble (LSTM + XGBoost + Transformer), automated execution & risk engine on AWS

---

## 🐍 Contribution Snake

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/PratikNighot/PratikNighot/output/github-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/PratikNighot/PratikNighot/output/github-snake.svg" />
  <img alt="github contribution snake" src="https://raw.githubusercontent.com/PratikNighot/PratikNighot/output/github-snake.svg" />
</picture>

## 🌐 Let's Connect!

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/pratik-nighot-37654582)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:pratiknighot52@gmail.com)

---

**Thanks for visiting!** ✨ Feel free to explore my repos or reach out for collaborations on **Backend Systems** or **Agentic AI**.
